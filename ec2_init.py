...
with Flow('install New Relic') as f:

    # cli_str1 = 'aws secretsmanager get-secret-value --secret-id  dev/eddp/nr_account_id'
    # cli_str1 = cli_str1+' --query SecretString --output text --region us-west-2'
    ...
    # cli_str2 = cli_str2+' --query SecretString --output text --region us-west-2'
    ...
    # cli_str = 'aws ecr get-login-password --region us-west-2 | docker login --username AWS '
    # cli_str = cli_str+'--password-stdin 091442718550.dkr.ecr.us-west-2.amazonaws.com/eddp' 
    # ecr_login = shell_task(command=cli_str, task_args=dict(name='docker pull',log_stdout=True))
    # ecr_login.set_upstream(nr_install)

    # cli_str = 'docker pull 091442718550.dkr.ecr.us-west-2.amazonaws.com/eddp:tag4' 
    # docker_pull = shell_task(command=cli_str, task_args=dict(name='docker pull',log_stdout=True))
    # docker_pull.set_upstream(ecr_login)

    # cli_str = 'docker run 091442718550.dkr.ecr.us-west-2.amazonaws.com/eddp:tag4' 
    ...
    cli_str = cli_str+' && chmod +x ./kubectl && mkdir -p $HOME/bin && cp ./kubectl $HOME/bin/kubectl '
    cli_str = cli_str+"&& export PATH=$PATH:$HOME/bin && echo 'export PATH=$PATH:$HOME/bin' >> ~/.bashrc"
    ...
    cli_str = '../../tmp/eksctl create cluster --name fargate-eks --region us-west-2 --fargate'
